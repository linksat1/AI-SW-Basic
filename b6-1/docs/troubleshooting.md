...
| 조치 내용 | 기존 `b6-1-web-server` 인스턴스를 선택해 종료(Terminate) — 해당 인스턴스는 오늘 다운로드한 키 페어(`b6-1-key-v2`)로는 접속 불가능한 상태였고 별도 데이터도 없어 삭제해도 무방하다고 판단 |
| 결과 | 새로 만든 인스턴스(`i-0c0d8cd61a76a5a99`) 하나만 남아 t3.micro 이중 과금 위험 제거 |
